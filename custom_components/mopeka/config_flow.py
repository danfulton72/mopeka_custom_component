...
from .const import (
    ...
    DOMAIN,
    MAX_REQUIRED_QUALITY,
    MIN_REQUIRED_QUALITY,
    MediumType,
)
...
def async_generate_schema(
    ...
    required_quality: int | None = None,
) -> vol.Schema:
    """Return the common schema for configuration and options."""
    return vol.Schema(
        {
            vol.Required(
        ...
        return self.async_show_form(
            ...
            data_schema=async_generate_schema(
                self.config_entry.options.get(
                    CONF_MEDIUM_TYPE,
                    self.config_entry.data.get(CONF_MEDIUM_TYPE, DEFAULT_MEDIUM_TYPE),
                ),
                self.config_entry.options.get(
                    CONF_REQUIRED_QUALITY,
